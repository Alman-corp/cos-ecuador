...
export class ConsultingDnaEngine {
  getRules(category?: DnaCategory): DnaRule[] {
    return category ? evaluationRules.filter((r) => r.category === category && r.enabled) : evaluationRules.filter((r) => r.enabled)
  }

  getThresholds(category?: DnaCategory): RiskThreshold[] {
    return category ? riskThresholds.filter((t) => t.category === category) : riskThresholds
  }

  getPatterns(category?: DnaCategory): RecommendationPattern[] {
    return category ? recommendationPatterns.filter((p) => p.category === category) : recommendationPatterns
  }

  getScales(category?: DnaCategory): MaturityScale[] {
    return category ? maturityScales.filter((s) => s.category === category) : maturityScales
  }
  ...
  evaluateMaturity(scaleId: string, score: number): { currentLevel: MaturityLevel; scale: MaturityScale | null; nextLevel: MaturityScale["levels"][0] | null; progress: number } {
  ...
  getMatchingPatterns(evaluations: { ruleId: string; triggered: boolean; value: number }[]): RecommendationPattern[] {
    const triggered = this.getRules().filter((r) => evaluations.find((e) => e.ruleId === r.id && e.triggered))
  ...
  getDnaSummary() {
    return {
      ...
      riskLevels: { low: 0, medium: 0, high: 0, critical: 0 } as Record<string, number>,
      patternsByPriority: {} as Record<string, number>,
      knowledgeByType: {} as Record<string, number>,
    }
  }
}

export const consultingDna = new ConsultingDnaEngine()
